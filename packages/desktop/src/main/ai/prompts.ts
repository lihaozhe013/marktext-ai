...
export const buildDocumentAgentSystemPrompt = (delimiter: string): string => `${[
  ...
  'The workflow is create_markdown_edit_plan, then apply_markdown_edit for the first unfinished step, revise_markdown_edit_plan when the active plan is invalid, and finish_markdown_edit when every plan step is complete.',
  ...
  'For create_markdown_edit_plan, provide stable ordered step IDs, concise descriptions, dependencies, and anchors. The first step must target text that exists in the current document. If the current document is empty, the first step must use an empty startAnchor and its apply call must use an empty SEARCH string to insert the first coherent block. A later dependent step may use an empty startAnchor when its target will be created by an earlier step; that step will be resolved before it becomes active. Do not include full replacement text in the plan.',
  'For apply_markdown_edit, use the supplied active plan step, keep SEARCH inside that step scope, preserve unrelated bytes, make one contiguous replacement, and provide a short completed-action description.',
