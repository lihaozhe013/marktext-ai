<template>
  <div class="pref-ai">
    ...
    <section class="ai-setting-group retry-setting">
      ...
      <select v-model.number="editAgentMaxSteps" @change="saveEditAgentMaxSteps">
    ...
    <div v-if="!settings.connections.length" class="empty-state">
    ...
    <div class="connection-layout">
      ...
      <section v-if="form.id || form.name" class="connection-editor">
        <div class="editor-heading">
          <h5>{{ form.id ? labels.editConnection : labels.newConnection }}</h5>
          <button v-if="form.id" class="danger-button" @click="deleteConnection">
        ...
        <section class="ai-setting-group">
          ...
          <select v-model="form.protocol">
            <option value="openai-chat-completions">OpenAI Chat Completions / Compatible</option>
        ...
        <section class="ai-setting-group">
          ...
          <input v-model="form.endpoint" type="url" autocomplete="url" placeholder="https://api.example.com/v1">
        ...
        <section class="ai-setting-group">
          ...
          <input
            ...
            :placeholder="selectedConnection?.hasApiKey ? labels.keyConfigured : labels.keyPlaceholder"
          >
        ...
        <section class="ai-setting-group">
          <div class="group-heading">
            ...
            <button class="secondary-button compact-button" :disabled="refreshing" @click="refreshModels">
              {{ refreshing ? labels.refreshing : labels.refreshModels }}
            </button>
          </div>
          <div v-for="model in form.models" :key="model.id || model.model" class="model-row">
            <input v-model="model.model" type="text" :placeholder="labels.modelId">
            <input v-model="model.label" type="text" :placeholder="labels.modelLabel">
            <button class="icon-danger" type="button" :title="labels.removeModel" @click="removeModel(model.id)">×</button>
          </div>
        ...
        <div class="ai-setting-actions">
          ...
          <button class="secondary-button" :disabled="testing" @click="test">
          ...
          <button class="danger-button" :disabled="!selectedConnection?.hasApiKey" @click="deleteKey">
            {{ labels.deleteKey }}
          </button>
        </div>
        <p v-if="status" class="status" :class="{ failure: !statusOk }">
          {{ status }}
        </p>
      </section>
    </div>
...
const labels = computed(() => chinese.value
  ? {
      title: 'AI 连接与模型',
      description: '配置多个兼容 OpenAI Chat Completions 或 Anthropic Messages 的连接。密钥只保存在本机用户数据目录，不会发送到渲染进程。',
      addConnection: '新增连接',
      ...
      endpointHint: '支持 Base URL 或完整端点，必须使用 HTTPS。模型列表接口不可用时仍可手动添加模型。',
      apiKey: 'API 密钥',
      keyPresent: '已配置密钥',
      keyMissing: '未配置密钥',
  ...
  : {
      ...
      description: 'Configure multiple connections compatible with OpenAI Chat Completions or Anthropic Messages. Keys stay in local user data and never enter the renderer.',
      ...
      endpointHint: 'Base URLs and complete endpoints are supported over HTTPS. Models can still be added manually when discovery is unavailable.',
      apiKey: 'API key',
      ...
      editAgentMaxStepsHint: 'Each step performs one local edit; reaching the limit discards the virtual edit.',
...
const selectConnection = (id: string): void => {
  ...
  defaultModelId.value = settings.value.defaultModel?.connectionId === connection.id
...
const saveEditAgentMaxSteps = async (): Promise<void> => {
  try {
    const value = await window.electron.ipcRenderer.invoke('mt::ai::set-edit-agent-max-steps', editAgentMaxSteps.value)
    applySettings(value)
    statusOk.value = true
...
const save = async (): Promise<void> => {
  ...
  try {
    ...
    connectionLog('save succeeded connectionId=%s modelCount=%s', saved.connections.find(connection => connection.id === (value.id ?? ''))?.models.length ?? value.models.length)
  } catch (err) {
    statusOk.value = false
    status.value = err instanceof Error ? err.message : String(err)
...
const addDiscoveredModel = (model: AiDiscoveredModel): void => {
  ...
  form.models.push({ id: createId(), model: model.model, label: model.label || model.model, source: 'discovered' })
}
...
const setDefault = async (): Promise<void> => {
  const model = form.models.find(item => item.id === defaultModelId.value && item.model.trim()) ??
...
<style scoped>
.pref-ai { max-width: 980px; }
.pref-ai .notes { max-width: 760px; line-height: 1.5; }
.connection-toolbar { display: flex; justify-content: flex-end; margin: 18px 0; }
.connection-layout { display: grid; grid-template-columns: minmax(180px, 240px) minmax(0, 1fr); gap: 18px; align-items: start; }
.connection-list { display: flex; flex-direction: column; gap: 8px; }
.connection-card { display: flex; flex-direction: column; gap: 4px; padding: 10px; border: 1px solid var(--floatBorderColor); border-radius: 5px; color: var(--editorColor); background: var(--editorBgColor); cursor: pointer; text-align: left; font: inherit; }
.connection-card.active { border-color: var(--highlightThemeColor); box-shadow: 0 0 0 1px var(--highlightThemeColor); }
.connection-card span, .connection-card small { color: var(--editorColor60); font-size: 12px; }
.connection-editor { min-width: 0; padding: 14px; border: 1px solid var(--floatBorderColor); border-radius: 6px; }
.editor-heading, .group-heading { display: flex; align-items: center; justify-content: space-between; gap: 10px; }
.editor-heading h5 { margin: 0; color: var(--editorColor); font-size: 16px; }
...
.ai-setting-group input, .ai-setting-group select { box-sizing: border-box; width: 100%; padding: 9px 10px; border: 1px solid var(--floatBorderColor); border-radius: 4px; background: var(--editorBgColor); color: var(--editorColor); font: inherit; }
.ai-setting-group small { color: var(--editorColor60); font-size: 12px; }
.model-row { display: grid; grid-template-columns: minmax(0, 1fr) minmax(0, 1fr) 28px; gap: 6px; align-items: center; }
.model-row + .model-row { margin-top: 7px; }
.model-row input { min-width: 0; }
.model-row button { height: 30px; }
.discovered-models { display: flex; flex-wrap: wrap; gap: 7px; margin: 12px 0; color: var(--editorColor); font-size: 12px; }
.discovered-model { padding: 5px 8px; border: 1px solid var(--floatBorderColor); border-radius: 4px; color: var(--editorColor); background: var(--editorBgColor); cursor: pointer; font: inherit; }
.discovered-model:disabled { cursor: default; opacity: .55; }
.ai-setting-actions { display: flex; flex-wrap: wrap; gap: 10px; margin-top: 28px; }
.primary-button, .secondary-button, .danger-button, .icon-danger { padding: 8px 14px; border: 0; border-radius: 4px; cursor: pointer; font: inherit; }
.compact-button { padding: 5px 8px; }
...
.icon-danger { padding: 3px 7px; color: #b63131; background: transparent; border: 1px solid currentColor; }
button:disabled { cursor: default; opacity: .45; }
.empty-state { padding: 18px; border: 1px dashed var(--floatBorderColor); color: var(--editorColor60); }
...
@media (max-width: 760px) {
  ...
  .connection-list { display: grid; grid-template-columns: repeat(auto-fit, minmax(150px, 1fr)); }
}
</style>
